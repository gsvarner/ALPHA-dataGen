...
Int_t eSmpl, nEvents = 10; // number of Events to process
...
if(Pending) {   
   ...
   if(tDelay==0) {
      ...
      printf("ADC complete at time %d\n",iTime);
      roc = ROB; // set readout counter to the number of bits to be handled
   }
}
...
if(RIP) { 
   ...
   nword = roc/16;  
   index = 15 - roc%16; // bit to be checked   
   if(HDR) {
   ...
   if(ADC) {
      // subtract off the 8 header words
   }
   if(FTR) {
   }   
   roc--; 
