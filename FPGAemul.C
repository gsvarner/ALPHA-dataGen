...
h[1][13][255]->Draw();
char plotName[100];
sprintf(plotName,"currently_1_13_255.png");
c1->Print(plotName);
...
if(Trigger) {
   ...
   printf("Processing Event %d at coarsetime %d, sample %d at Readout starting Sample = %d\n",dEvent,evtCCtime,evtSmpl,startSmpl);
   // fill header info
   for(Int_t iBank=0; iBank<nBank; iBank++) {
      for(Int_t iSmpl=0; iSmpl<nSmpl; iSmpl++) {
         in >> dSmpl;
...
// send header
   if(HDR) {
      if(nword == 8 && index == 0) {
         HDR = 0; 
         ADC = 1; // move to ADC data
         printf("Header complete at time %d\n",iTime);         
      } 
...
// send data
   if(ADC) {
      // subtract off the 8 header words -- to see if done
      curWord = nword-8;
      if( ((nword-8) == 16*Nsamples) && (index == 0) ) {
         ADC = 0; 
         FTR = 1; // move to Footer
         printf("ADC complete at time %d\n",iTime); 
      } 
...
// send footer   
   if(FTR) {
       ...
       if(nword == (8 + 16*Nsamples + 1) ) {
          ...
          printf("Footer/event complete at time %d\n",iTime); 
       }
   }
}      
